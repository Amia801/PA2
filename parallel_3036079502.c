...
typedef struct {
    ...
    int task_front;
    int task_rear;
    int pending_tasks;            // 待完成任务数
    pthread_mutex_t lock;
    pthread_cond_t cond;
    pthread_cond_t tasks_done;    // 所有任务完成的条件变量
    bool stop;
} thread_pool_t;
...
void *thr_func(void *arg) {
    while (true) {
        ...
        if (pool->pending_tasks == 0) {
            pthread_cond_signal(&pool->tasks_done); // 所有任务完成
        }
        pthread_mutex_unlock(&pool->lock);
    }
    return NULL;
...
void add_task_to_pool(void (*function)(void*), void* arg) {
    ...
    if (pool->task_count < MAX_TASKS) {
        ...
        pool->task_count++;
        pool->pending_tasks++;  // 增加待完成任务计数

        pthread_cond_signal(&pool->cond);  // 唤醒一个等待的线程
    }
